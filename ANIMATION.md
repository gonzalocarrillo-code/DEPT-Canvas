...
- **Easing is a fixed enum** (via `setEnum` on `animationEasing`). Phase 0 (v1.76.1) found **16 values** — `Linear`/`EaseIn`/`EaseOut`/`EaseInOut` **plus Quart/Quint/Back/Spring variants** — so overshoot/spring *feel* IS reachable on Tier 1. There is still **no custom cubic-bezier** (you cannot supply control points) and **no per-keyframe easing**. Read the live list from the engine; never hardcode it.
...
- Motion = **composition of CE.SDK presets**. The motion-primitives library combines In/Out/Loop presets, staggers (via time offsets), and the engine's easing enum (16 values on v1.76.1, incl. Back/Spring) to approximate richer motion.
...
CAN: pick a preset intent and parameterise it; stagger layers via time offsets; choose any of the engine's easings (16 on v1.76.1, incl. Back/Spring for overshoot feel); combine presets into a `composed` result; sequence In/Out across a timeline.
